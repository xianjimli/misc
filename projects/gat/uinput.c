...
UInput* uinput_create(const char* dev, int xmax, int ymax)
{
	...
	strncpy(uinp.name, "qwerty2", UINPUT_MAX_NAME_SIZE);
...
void uinput_report_event(UInput* thiz, struct input_event* event)
{
	if(thiz != NULL && thiz->fd > 0)
	{
		write(thiz->fd, event, sizeof(struct input_event));
	}
