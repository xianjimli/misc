...
static CIniGroups* ini_parse_internal(char* buffer, char comment_char, char delim_char)
{
	char* p = buffer;
			...
			case STAT_GROUP:
			{
				if(*p == ']')
				{
					*p = '\0';
					...
					current_group = groups->Find(group_start);
					state = STAT_NONE;
				}
				break;
			}
				...
				if(*p == '\n' || *p == '\r')
				{
					*p = '\0';
					if(current_group == NULL)
					{
						...
						current_group = groups->Find(DEFAULT_GROUP);
					}
					strtrim(key_start);
					strtrim(value_start);
					current_group->GetPairs().Append(key_start, value_start);
					state = STAT_NONE;
				}
				break;
			}
...
CIniGroups* ini_parse(const char* file_name, char comment_char, char delim_char)
{
	FILE* fp = NULL;
	struct stat st = {0};
	char* buffer = NULL;
...
void parser_test1(void)
{
	...
	CIniGroups* groups = ini_parse_internal(buffer, '#', '=');

	groups->Save(stdout, '=');
	assert(groups->Size() == 2);
	assert(groups->Find("lixianjing")->GetName() == "lixianjing");
...
{
	CIniGroups* groups = ini_parse(file_name, '#', delim);
