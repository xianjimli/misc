...
JRet arm7_99_execute(JInstruction* thiz, jinst_t instr)
{
	J_DECL_MACHINE(thiz);
	j_ret_val_if_fail(m != NULL, JRET_INVALID_PARAM);

	jword_t addr = J_LHS + 4;
	arm7_load_mult(m, instr, addr, 0L);

	return JRET_OK;
}

JRet arm7_99_das(JInstruction* thiz, jinst_t instr, char* t)
{
	char* p = NULL;
	jword_t r = 0;
	j_ret_val_if_fail(t != NULL, JRET_INVALID_PARAM);
	sprintf(t, "ldmib %s%s, {", arm7_r_name(J_RLHS(instr)),
		J_BIT(instr, 21) ? "!" : "");
...
JRet arm7_99_as(JInstruction* thiz, const char* t, jinst_t* i)
{
...
void arm7_99_test(void)
{
	char str[256] = {0};
	jword_t result = 0;
	jword_t text[12] = {0xe9940003};
	jword_t data[4] = {0x11111111, 0x22222222};

	JInstruction* thiz = ARM7_INSTR_CREATE(99);
