...
JRet arm7_5d_execute(JInstruction* thiz, jinst_t instr)
{
	J_DECL_MACHINE(thiz);
	j_ret_val_if_fail(m != NULL, JRET_INVALID_PARAM);
	
	jword_t data = jmachine_get_byte(m, J_LHS + J_BITS(instr, 0,11));
	J_R_WR(m, J_RDEST(instr), data);

	return JRET_OK;
}

JRet arm7_5d_das(JInstruction* thiz, jinst_t instr, char* t)
{
...
JRet arm7_5d_as(JInstruction* thiz, const char* t, jinst_t* i)
{
...
void arm7_5d_test(void)
{
	char str[256] = {0};
	jword_t text[12] = {0xe5d23000, 0xe5d23004};
	jword_t data[4]  = {0x11111111, 0x22222222};

	JInstruction* thiz = ARM7_INSTR_CREATE(5d);
