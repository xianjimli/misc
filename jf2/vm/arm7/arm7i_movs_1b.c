...
JRet arm7_1b_execute(JInstruction* thiz, jinst_t instr)
{
	J_DECL_MACHINE(thiz);
	j_ret_val_if_fail(m != NULL, JRET_INVALID_PARAM);

	J_R_WR(m, J_RDEST(instr), J_DPSRHS);

	return JRET_OK;
}

JRet arm7_1b_das(JInstruction* thiz, jinst_t instr, char* t)
{
...
JRet arm7_1b_as(JInstruction* thiz, const char* t, jinst_t* i)
{
