...
JRet arm7_90_execute(JInstruction* thiz, jinst_t instr)
{
	J_DECL_MACHINE(thiz);
...
JRet arm7_90_das(JInstruction* thiz, jinst_t instr, char* t)
{
	char* p = NULL;
	jword_t r = 0;
	j_ret_val_if_fail(t != NULL, JRET_INVALID_PARAM);
	sprintf(t, "stmdb %s%s, {", arm7_r_name(J_RLHS(instr)),
		J_BIT(instr, 21) ? "!" : "");
...
JRet arm7_90_as(JInstruction* thiz, const char* t, jinst_t* i)
{
...
void arm7_90_test(void)
{
	char str[256] = {0};
	jword_t result = 0;
	jword_t text[12] = {0xe90d4001};
	jword_t data[4] = {0};

	JInstruction* thiz = ARM7_INSTR_CREATE(90);
	JMachine* m = jmachine_create(text, sizeof(text), 0);
	
	m->r[J_RSP] = (size_t)(data + 2);
	m->r[0] = 0x11111111;
	m->r[J_RLR] = 0x22222222;
