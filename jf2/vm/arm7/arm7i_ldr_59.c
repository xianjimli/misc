...
JRet arm7_59_execute(JInstruction* thiz, jinst_t instr)
{
	J_DECL_MACHINE(thiz);
	j_ret_val_if_fail(m != NULL, JRET_INVALID_PARAM);

	jword_t data = jmachine_get_word(m, J_LHS + J_BITS(instr, 0,11));	
	J_R_WR(m, J_RDEST(instr), data);

	return JRET_OK;
}

JRet arm7_59_das(JInstruction* thiz, jinst_t instr, char* t)
{
	j_ret_val_if_fail(t != NULL, JRET_INVALID_PARAM);

	if(J_BITS(instr, 0,11) != 0)
	{
		sprintf(t, "ldr%s %s, [%s, #%d]; 0x%x\n",
			arm7_cond_name(J_ARM_COND(instr)),
			arm7_r_name(J_RDEST(instr)),
			arm7_r_name(J_RLHS(instr)),
			J_BITS(instr, 0,11), J_BITS(instr, 0,11));
	}
	else
	{
		sprintf(t, "ldr%s %s, [%s]\n",
...
JRet arm7_59_as(JInstruction* thiz, const char* t, jinst_t* i)
{
...
void arm7_59_test(void)
{
	char str[256] = {0};
	jword_t result = 0;
	jword_t text[12] = {0xe5940000, 0xe5940004};
	jword_t data[4] = {0x11111111, 0x22222222};

	JInstruction* thiz = ARM7_INSTR_CREATE(59);
