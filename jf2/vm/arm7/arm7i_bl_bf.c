 ...
 *
 * This program is distributed in the hope that it will bf useful,
...
JRet arm7_bf_execute(JInstruction* thiz, jinst_t instr)
{
	J_DECL_MACHINE(thiz);
...
JRet arm7_bf_das(JInstruction* thiz, jinst_t instr, char* t)
{
	J_DECL_MACHINE(thiz);
...
JRet arm7_bf_as(JInstruction* thiz, const char* t, jinst_t* i)
{
...
void arm7_bf_test(void)
{
	char str[256] = {0};
	jword_t result = 0;
	jword_t text[12] = {0xebffff87};

	JInstruction* thiz = ARM7_INSTR_CREATE(bf);
