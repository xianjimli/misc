...
JRet arm7_3a_execute(JInstruction* thiz, jinst_t instr)
{
	J_DECL_MACHINE(thiz);
...
JRet arm7_3a_das(JInstruction* thiz, jinst_t instr, char* t)
{
	j_ret_val_if_fail(t != NULL, JRET_INVALID_PARAM);
	jword_t data = arm7_decode_immed(J_BITS(instr, 0, 11));
		
	sprintf(t, "mov%s %s, #%d ; 0x%x\n", 
...
JRet arm7_3a_as(JInstruction* thiz, const char* t, jinst_t* i)
{
...
void arm7_3a_test(void)
{
	char str[256] = {0};
	jword_t result = 0;
	jword_t text[12] = {0xe3a0d702};

	JInstruction* thiz = ARM7_INSTR_CREATE(3a);
