...
JRet arm7_8b_execute(JInstruction* thiz, jinst_t instr)
{
	J_DECL_MACHINE(thiz);
	j_ret_val_if_fail(m != NULL, JRET_INVALID_PARAM);

	jword_t base = J_LHS + arm7_count_one(J_BITS(instr, 0, 15))*4;
	arm7_load_mult(m, instr, J_LHS, base);

	return JRET_OK;
}

JRet arm7_8b_das(JInstruction* thiz, jinst_t instr, char* t)
{
	char* p = NULL;
	jword_t r = 0;
...
JRet arm7_8b_as(JInstruction* thiz, const char* t, jinst_t* i)
{
...
void arm7_8b_test(void)
{
	char str[256] = {0};
	jword_t result = 0;
	jword_t text[12] = {0xe8b34002};
	jword_t data[4] = {0x11111111, 0x22222222};

	JInstruction* thiz = ARM7_INSTR_CREATE(8b);
