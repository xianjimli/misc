...
JRet arm7_1e_execute(JInstruction* thiz, jinst_t instr)
{
	J_DECL_MACHINE(thiz);
	j_ret_val_if_fail(m != NULL, JRET_INVALID_PARAM);
	
	jword_t data = ~(J_DPRHS);
	J_R_WR(m, J_RDEST(instr), data);

	return JRET_OK;
}

JRet arm7_1e_das(JInstruction* thiz, jinst_t instr, char* t)
{
	J_DECL_MACHINE(thiz);
...
JRet arm7_1e_as(JInstruction* thiz, const char* t, jinst_t* i)
{
...
void arm7_1e_test(void)
{
	char str[256] = {0};
	jword_t result = 0;
	jword_t text[12] = {0xe1e00000};
	jword_t data[4] = {0x11111111};

	JInstruction* thiz = ARM7_INSTR_CREATE(1e);
