...
JRet arm7_00_execute(JInstruction* thiz, jinst_t instr)
{
	J_DECL_MACHINE(thiz);
	...
	{	
		J_R_WR(m, J_RDEST(instr), J_LHS & J_RHS);
	}
	else
	{
		if(J_RMULLHS(instr) == J_RMULDEST(instr))
		{
			J_R_WR(m, J_RMULDEST(instr), 0);
		}
		else if(J_RMULDEST(instr) != 15)
		{
			J_R_WR(m, J_RMULDEST(instr), J_MULLHS * J_MULRHS);
		}
	}
...
JRet arm7_00_das(JInstruction* thiz, jinst_t instr, char* t)
{
	j_ret_val_if_fail(t != NULL, JRET_INVALID_PARAM);
	
	if (J_BITS (instr, 4, 7) != 9)
	{
		sprintf(t, "and%s %s, %s, %s\n", 
	...
	else
	{
		...
		{
			sprintf(t, "mul%s %s, %s, %s\n",
				arm7_cond_name(J_ARM_COND(instr)),
				arm7_r_name(J_RMULDEST(instr)),
				arm7_r_name(J_RMULLHS(instr)),
				arm7_r_name(J_RMULRHS(instr)));
		}
	}
...
JRet arm7_00_as(JInstruction* thiz, const char* t, jinst_t* i)
{
...
void arm7_00_test(void)
{
	char str[256] = {0};
	jword_t result = 0;
	jword_t text[12] = {0xe0023003, 0xe0030392};

	JInstruction* thiz = ARM7_INSTR_CREATE(00);
