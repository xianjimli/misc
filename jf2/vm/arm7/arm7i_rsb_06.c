...
JRet arm7_06_execute(JInstruction* thiz, jinst_t instr)
{
	J_DECL_MACHINE(thiz);
...
JRet arm7_06_das(JInstruction* thiz, jinst_t instr, char* t)
{
	j_ret_val_if_fail(t != NULL, JRET_INVALID_PARAM);
	sprintf(t, "rsb%s %s, %s, %s\n",
...
JRet arm7_06_as(JInstruction* thiz, const char* t, jinst_t* i)
{
...
void arm7_06_test(void)
{
	char str[256] = {0};
	jword_t result = 0;
	jword_t text[12] = {0xe0633002};
	jword_t data[4]  = {0};

	JInstruction* thiz = ARM7_INSTR_CREATE(06);
