...
JRet arm7_92_execute(JInstruction* thiz, jinst_t instr)
{
	J_DECL_MACHINE(thiz);
	j_ret_val_if_fail(m != NULL, JRET_INVALID_PARAM);

	jword_t addr = J_LHS - arm7_count_one(J_BITS(instr, 0, 15))*4;
	arm7_store_mult(m, instr, addr, addr);

	return JRET_OK;
}

JRet arm7_92_das(JInstruction* thiz, jinst_t instr, char* t)
{
	char* p = NULL;
	jword_t r = 0;
...
JRet arm7_92_as(JInstruction* thiz, const char* t, jinst_t* i)
{
...
void arm7_92_test(void)
{
	char str[256] = {0};
	jword_t result = 0;
	jword_t text[12] = {0xe92dd800};
	jword_t data[4] = {0};

	JInstruction* thiz = ARM7_INSTR_CREATE(92);
	JMachine* m = jmachine_create(text, sizeof(text), 0);
	
	m->r[J_RSP] = (size_t)(data + 4);
