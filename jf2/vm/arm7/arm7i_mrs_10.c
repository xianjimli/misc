...
JRet arm7_10_execute(JInstruction* thiz, jinst_t instr)
{
	J_DECL_MACHINE(thiz);
	...
	{
		jword_t addr = J_LHS;
		jword_t data = arm7_swap_word(m, addr, J_RHS);

		if(addr & 3)
		{
			data = arm7_align(m, addr, data);
		}
	...
	{
		jword_t cpsr = J_ECC | J_EINT | J_EMODE;
		J_R_WR(m, J_RDEST(instr), cpsr);
	}

	return JRET_OK;
}

JRet arm7_10_das(JInstruction* thiz, jinst_t instr, char* t)
{
...
JRet arm7_10_as(JInstruction* thiz, const char* t, jinst_t* i)
{
...
void arm7_10_test(void)
{
	char str[256] = {0};
	jword_t result = 0;
	jword_t text[12] = {0xe10f2000, 0xe1013092};
	jword_t data[6] =  {0x11111111, 0x22222222};
	JInstruction* thiz = ARM7_INSTR_CREATE(10);
