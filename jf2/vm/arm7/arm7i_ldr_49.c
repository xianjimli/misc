...
JRet arm7_49_execute(JInstruction* thiz, jinst_t instr)
{
	J_DECL_MACHINE(thiz);
	j_ret_val_if_fail(m != NULL, JRET_INVALID_PARAM);

	jword_t data = jmachine_get_word(m, J_LHS);	
...
JRet arm7_49_das(JInstruction* thiz, jinst_t instr, char* t)
{
	j_ret_val_if_fail(t != NULL, JRET_INVALID_PARAM);

	if(J_BITS(instr, 0,11) != 0)
	{
		sprintf(t, "ldr%s %s, [%s], #%d\n",
...
JRet arm7_49_as(JInstruction* thiz, const char* t, jinst_t* i)
{
...
void arm7_49_test(void)
{
	char str[256] = {0};
	jword_t result = 0;
	jword_t text[12] = {0xe4984004};
	jword_t data[4] = {0x11111111, 0x22222222};

	JInstruction* thiz = ARM7_INSTR_CREATE(49);
