...
JRet arm7_25_execute(JInstruction* thiz, jinst_t instr)
{
	J_DECL_MACHINE(thiz);
	j_ret_val_if_fail(m != NULL, JRET_INVALID_PARAM);
	jword_t lhs = J_LHS;
	jword_t rhs = arm7_decode_immed(J_BITS(instr, 0, 11));
	jword_t data = lhs - rhs;
	if ((lhs >= rhs) || ((rhs | lhs) >> 31))
	{
		arm7_subcarry(m, lhs, rhs, data);
		arm7_sub_overflow(m, lhs, rhs, data);
	}
	else
	{
		J_CLEARC;
...
JRet arm7_25_das(JInstruction* thiz, jinst_t instr, char* t)
{
	j_ret_val_if_fail(t != NULL, JRET_INVALID_PARAM);
	jword_t rhs = arm7_decode_immed(J_BITS(instr, 0, 11));

	sprintf(t, "subs %s, %s, #%d  ; 0x%x\n",
		arm7_r_name(J_RDEST(instr)),
		arm7_r_name(J_RLHS(instr)), rhs, rhs);


	return JRET_OK;
}

JRet arm7_25_as(JInstruction* thiz, const char* t, jinst_t* i)
{
...
void arm7_25_test(void)
{
	char str[256] = {0};
	jword_t result = 0;
	jword_t text[12] = {0xe2503000};
	jword_t data[4] = {0};

	JInstruction* thiz = ARM7_INSTR_CREATE(25);
