...
JRet arm7_4c_execute(JInstruction* thiz, jinst_t instr)
{
	J_DECL_MACHINE(thiz);
	j_ret_val_if_fail(m != NULL, JRET_INVALID_PARAM);
	
	jmachine_put_byte(m, J_LHS, J_DEST);
	J_R_WR(m, J_RLHS(instr), J_LHS + J_BITS(instr, 0,11));

	return JRET_OK;
}

JRet arm7_4c_das(JInstruction* thiz, jinst_t instr, char* t)
{
...
JRet arm7_4c_as(JInstruction* thiz, const char* t, jinst_t* i)
{
...
void arm7_4c_test(void)
{
	char str[256] = {0};
	jword_t result = 0;
	jword_t text[12] = {0xe4c30001};
	jword_t data[4] = {0};

	JInstruction* thiz = ARM7_INSTR_CREATE(4c);
