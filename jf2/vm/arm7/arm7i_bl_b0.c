...
JRet arm7_b0_execute(JInstruction* thiz, jinst_t instr)
{
	J_DECL_MACHINE(thiz);
	j_ret_val_if_fail(m != NULL, JRET_INVALID_PARAM);

	jsword_t offset = J_POS_BL(instr);
	m->r[J_RLR] = m->pc + 4;
...
JRet arm7_b0_das(JInstruction* thiz, jinst_t instr, char* t)
{
	J_DECL_MACHINE(thiz);
	j_ret_val_if_fail(t != NULL, JRET_INVALID_PARAM);

	jword_t target = m->pc + 8 + J_POS_BL(instr);
...
JRet arm7_b0_as(JInstruction* thiz, const char* t, jinst_t* i)
{
...
void arm7_b0_test(void)
{
	char str[256] = {0};
	jword_t result = 0;
	jword_t text[12] = {0xeb0003ed};

	JInstruction* thiz = ARM7_INSTR_CREATE(b0);
